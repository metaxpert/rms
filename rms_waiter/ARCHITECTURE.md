...
| 9 | CI: the eight phases' checks, run by something other than a person remembering | **Done, verified** |
...
### Phase 9 — the checks stop depending on memory

Every phase above was validated by running `flutter analyze` and `flutter test`
by hand, in five packages, in order. That is not a gate; it is a habit, and the
checks that repay the most are exactly the ones nobody thinks to run — the i18n
parity test that catches a screen going half-Urdu mid-sentence, and the
orphaned-key scan that protects a translator's time. Both were written in Phase
8 and neither ran unless asked.

`.github/workflows/ci.yml` and `scripts/check.sh` are the same steps, one for a
laptop and one for a push, and each says so at the top so they cannot quietly
drift apart. `rms_core` is its own job that the four apps wait on: it is a path
dependency of all of them, so a break there should be reported once rather than
five times.

Two checks were added that were not previously run anywhere:

- **l10n drift.** The generated localisations are committed *and* regenerated by
  every build. An `.arb` edited without regenerating therefore leaves a stale
  generated file in git that is invisible to whoever did it — their next build
  rewrites it locally — and lands as an unrelated diff in someone else's branch.
  CI regenerates all five catalogues and requires git to see nothing.
- **`flutter build web --release`.** `flutter test` runs on the Dart VM and never
  compiles for the web. dart2js rejects things the VM accepts, and the four apps
  are demonstrated in a browser, so the build is the only thing that catches it.

The gate was verified by making it fail, not only by watching it pass: a probe
key added to one app's `.arb` files was caught twice over, by the orphaned-key
test and by the drift check, and the summary named both.

`dart format` is deliberately **not** enforced. The tree is not format-clean —
53 files would change — and reformatting them belongs in its own commit, decided
on its own merits, rather than riding in with the CI that would start requiring it.

The Flutter version is pinned rather than tracking `stable`, because `stable`
