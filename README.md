...
One command runs everything CI asserts, across all five packages:

```bash
bash scripts/check.sh            # pub get + analyze + test + l10n drift
bash scripts/check.sh --build    # ...and a release web build of each app
bash scripts/check.sh rms_waiter # one package only
```

`.github/workflows/ci.yml` runs the same steps on every push and pull request — `rms_core` first,
then the four apps in parallel, then the web builds. Add a step to one, add it to the other.

Two of the checks are less obvious than "the tests pass":

- **l10n drift.** The generated localisations are committed *and* regenerated by every build, so an
  `.arb` edited without regenerating leaves a stale file in git that is invisible to whoever did it —
  their next build rewrites it locally — and surfaces as an unrelated diff in someone else's branch.
  CI regenerates all five catalogues and insists git sees nothing.
- **`flutter build web`.** `flutter test` runs on the VM and never compiles for the web; dart2js
  rejects things the VM accepts. The build is the only thing that catches it.

The Flutter version is pinned in the workflow. Bump it deliberately, in its own commit, with the
fallout fixed in that commit — `stable` gains lints between releases, and a gate that fails on a
morning when nobody changed anything is a gate people learn to ignore.
